const round = (val:any, precision = 2) => {
...
export class LocalCart {
  ...
  get lineItems() {
    return this.state.lineItems.map((x:any) => Object.assign({}, x));
  }
  ...
  *eachItem() {
  ...
  calculateSubTotal() {
    ...
    this.state.lineItems.forEach((li:any) => {
  ...
  getVariantQuantity(variantId:any) {
  ...
  setVariantQuantity(variantId:any, qty:number) {
    for (let i = 0; i < this.state.lineItems.length; i++) {
      const li = this.state.lineItems[i];
  ...
  hasVariant(variantId:any) {
    return this.state.lineItems.map((x:any) => parseFloat(x.variantId)).indexOf(parseFloat(variantId)) > -1;
  }

  removeLineItem(variantId:any) {
    this.state.lineItems = this.state.lineItems.filter((x:any) => parseFloat(x.variantId) != parseFloat(variantId));
    this.calculate();
  }
  ...
  setFreeShippingMinimum(freeShippingMinimum:number) {
    this.state.freeShippingMinimum = freeShippingMinimum;
    this.calculate();
  }

  setShippingSurcharge(amount:number) {
    this.state.shippingSurcharge = amount;
    this.calculate();
  }

  setDiscount(discountType:any, discountValue:any, freeShipping:any) {
  ...
  addLineItem(variantId:any, title:any, subtitle:any, price:any, quantity:any, shippingRatePerItem:any, discountable:boolean = true, compareAtPrice:any = null, productImage:any=null) {
    price               = parseFloat(price);
    quantity            = parseInt(quantity);
    compareAtPrice      = parseFloat(compareAtPrice)      || null;
    shippingRatePerItem = parseFloat(shippingRatePerItem) || 0.0;
    this.state.lineItems.push({ variantId, title, subtitle, price, quantity, shippingRatePerItem, discountable, compareAtPrice, productImage });
    this.calculate();
  }

  setTax(taxDescription:any, taxRate:any, taxTotal:any) {
...
LocalCart.fromJSON = (jsonStr:string) => {
