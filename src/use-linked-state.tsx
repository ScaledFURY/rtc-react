...
export function useStateGateway(initialState:any) {
  const gateway : any = useRef({ members: [], initialState })
  useEffect(() => {
    return () => {
...
//
export function useLinkedState(gateway:any) {
  const initialState = gateway.current?.hasOwnProperty('lastUpdate') ? gateway.current.lastUpdate : gateway.current?.initialState;
